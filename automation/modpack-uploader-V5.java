...
public class ModpackTool {
    ...
    public static void main(String[] args) throws Exception {
        ...
        if (FLAGS.releaseOnly) {
            step("GitHub release", ModpackTool::createGitHubRelease);
            log("✔ Done");
            return;
        }

        if (!FLAGS.serverOnly && flag("ENABLE_CLIENT_FILE_MODULE")) {
            step("Build client", ModpackTool::buildClient);

            if (!FLAGS.skipUpload && flag("ENABLE_MODPACK_UPLOADER_MODULE")) {
                int id = step("Upload client", ModpackTool::uploadClient);

                if (!FLAGS.clientOnly && flag("ENABLE_SERVER_FILE_MODULE")) {
        ...
        if (FLAGS.serverOnly && flag("ENABLE_SERVER_FILE_MODULE")) {
            step("Build server", ModpackTool::buildServer);

            if (!FLAGS.skipUpload) {
                step("Upload server", () -> uploadServer(-1));
            }
        }

        if (!FLAGS.clientOnly && !FLAGS.serverOnly && flag("ENABLE_GITHUB_RELEASE_MODULE")) {
            step("GitHub release", ModpackTool::createGitHubRelease);
        }
    ...
    static void parseArgs(String[] args) {
        for (String arg : args) {
            switch (arg) {
                case "--dry-run" -> FLAGS.dryRun = true;
                case "--skip-upload" -> FLAGS.skipUpload = true;
                case "--client-only" -> FLAGS.clientOnly = true;
                case "--server-only" -> FLAGS.serverOnly = true;
                case "--release-only" -> FLAGS.releaseOnly = true;
